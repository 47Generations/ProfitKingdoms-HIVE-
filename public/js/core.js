...
function renderInventory() {
  ...
  for (let i = 0; i < inventory.slots; i++) {
    ...
    if (i < itemEntries.length) {
      const [itemId, qty] = itemEntries[i];
      const itemDef = ITEM_DEFS[itemId];
      if (!itemDef) continue;

      // Attach item reference for context menu
      slotDiv.dataset.itemId = itemId;

      slotDiv.addEventListener("contextmenu", (e) => {
        e.preventDefault();
        showInventoryMenu(e.pageX, e.pageY, itemId);
      });
...
function showInventoryMenu(x, y, itemId) {
  contextItemId = itemId;

  inventoryMenu.style.left = `${x}px`;
  inventoryMenu.style.top = `${y}px`;
